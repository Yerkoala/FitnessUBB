...
import ejerciciosNombre from "../ejerciciosNombre"
...
export function RutinasProvider({ children }) {
        ...
        Abdomen: [
            {
                ...
                ejercicios: ["Crunch", "Crunch oblicuo", "Abdominales en máquina", "Abdominales con cuerda en polea alta", "Elevación de piernas"],
            },
            {
                nombre: "Rutina Abdomen 2",
                ejercicios: ["Encogimientos de rodillas para abdominales", "Abdominales con brazos estirados", "Plancha con flexión", "Abdominales en máquina", "Crunch oblicuo"],
            },
        ],
        Pierna: [
            {
                nombre: "Rutina Pierna 1",
                ejercicios: ["Sentadilla", "Prensa de piernas", "Extensión de piernas", "Aperturas con mancuernas", "Sentadilla Hack", "Curl de piernas sentado"],
            },
            {
                nombre: "Rutina Pierna 2",
                ejercicios: ["Extensión a una pierna", "Sentadilla frontal", "Peso muerto rumano (piernas rectas) con mancuernas", "Prensa de piernas", "Sentadilla"],
            },
        ],
        Gluteo: [
            {
                nombre: "Rutina Gluteo 1",
                ejercicios: ["Sentadilla", "Sentadilla búlgara con barra", "Sentadilla con mini banda", "Peso muerto con balón medicinal", "Hip thrust con maquina Smith"],
            },
            {
                nombre: "Rutina Gluteo 2",
                ejercicios: ["Sentadilla Hack", "Sentadillas con propio peso", "Hip thrust con barra", "Levantamiento de pierna acostado de lado", "Contragolpe con cable"],
            },
        ],
        Todos: [
            {
                nombre: "Lista de ejercicios",
                ejercicios: ejerciciosNombre
            }
        ]
    }
    )
    ...
    //Almacenamiento en localStorage para que al recargar la app el objeto se mantenga actualizado
    useEffect(() => {
        const storedRutinaObjeto = localStorage.getItem("rutinaObjeto");
        if (storedRutinaObjeto) {
            const parsedRutinaObjeto = JSON.parse(storedRutinaObjeto);
            if (JSON.stringify(parsedRutinaObjeto) !== JSON.stringify(rutinaObjeto)) {
                setRutinaObjeto(parsedRutinaObjeto);
            }
        }
    }, []);

    useEffect(() => {
        localStorage.setItem("rutinaObjeto", JSON.stringify(rutinaObjeto));
    }, [rutinaObjeto]);
